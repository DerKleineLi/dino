...
def extract_feature_pipeline(args):
    ...
    if args.subset > 0:
        np.random.seed(0)
        ids = np.random.permutation(len(dataset_train))[: args.subset]
        dataset_train = ReturnIndexSubset(dataset_train, ids)
...
class ReturnIndexSubset(torch.utils.data.Subset):
    def __init__(self, dataset, ids):
        super().__init__(dataset, ids)
        self.samples = [dataset.samples[id] for id in ids]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", default="29502", type=str, help="port for distributed_mode."
    )
    parser.add_argument(
        "--subset",
        default=0,
