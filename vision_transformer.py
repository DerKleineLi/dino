...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, dropout_patches=False, debug=False, class_token_only=True):
    ...
    def drop_patches(self, x):
        ...
        B, P, D = x.shape
        P -= 1
        out_P = int(P * keep_rate)
        output = torch.zeros(B, out_P + 1, D, device=x.device)
        output[:, 0] = x[:, 0]
        for i in range(B):
            perm = torch.randperm(P)[:out_P] + 1
            output[i, 1:] = x[i, perm]
        return output
